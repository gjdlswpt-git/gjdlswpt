...
<?php if($msn) { ?>MSN：<a href="msnim:chat?contact=<?php echo $msn;?>" rel="nofollow"><?php echo $msn;?></a><br/><?php } ?>
<?php if($qq) { ?>QQ：<a href="tencent://message/?uin=<?php echo $qq;?>&Site=<?php echo $title;?>&Menu=yes" rel="nofollow"><img src="http://wpa.qq.com/pa?p=1:<?php echo $qq;?>:17" width="25" height="17" title="点击QQ交谈/留言" alt=""/> <?php echo $qq;?></a><br/><?php } ?>
</div>
...
<span class="px12"><strong>参考资料：</strong><a href="<?php echo fix_link($best['linkurl']);?>" target="_blank" rel="nofollow"><?php echo $best['linkurl'];?></a></span>
<?php } ?>
</div>
<?php } else if($module == 'down') { ?>
<div class="downurl">
<ul>
<li><a href="<?php echo $MOD['linkurl'];?>down.php?auth=<?php echo $auth;?>" class="t" rel="nofollow">主站下载</a></li>
<?php if($MIRROR) { ?>
<?php if(is_array($MIRROR)) { foreach($MIRROR as $k=>$v) { ?>
<li><a href="<?php echo $MOD['linkurl'];?>down.php?mirror=<?php echo $k;?>&auth=<?php echo $auth;?>" class="t" rel="nofollow"><?php echo $v['name'];?>镜像</a></li>
<?php } } ?>
<?php } ?>
</ul>
<div class="c_b"></div>
...
<a href="<?php echo $v['linkurl'];?>#p"><img src="<?php echo $v['thumb'];?>" width="80" height="80" title="<?php echo $v['introduce'];?>" alt="" <?php if($page==$v['page']) { ?>class="thumb_b"<?php } else { ?>class="thumb_a" onmouseover="this.className='thumb_b';" onmouseout="this.className='thumb_a';"<?php } ?>
/></a>
<?php } } ?>
<?php } else { ?>
<div><img src="<?php echo DT_SKIN;?>image/spacer.gif" id="cursor_a" onclick="Go('<?php echo $prev_photo;?>#p');" title="上一张 支持键盘←方向键"/></a><img src="<?php echo DT_SKIN;?>image/spacer.gif" id="cursor_b" onclick="Go('<?php echo $next_photo;?>#p');" title="下一张 支持键盘→方向键"/></div>
...
<div class="user_reg"><a href="<?php echo $MODULE['2']['linkurl'];?><?php echo $DT['file_register'];?>" rel="nofollow"><img src="<?php echo DT_SKIN;?>image/user_reg.gif" width="260" height="26" alt="还不是会员，立即免费注册"/></a></div>
